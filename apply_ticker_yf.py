...
def main():
    ...
    # ABSOLUTE cash ceiling (root data fix), YAHOO-ANCHORED. The directional
    # reconcile above only pulls a stale-LOW master cash UP to Yahoo; the "broad
    # basis" defense lets master cash EXCEED Yahoo's narrow totalCash with NO
    # upper bound — so a corrupted cash survives and poisons EV, net_debt_ebitda,
    # net_cash_pct_mcap and every net-cash archetype (YAR.OL: cash 1.05T NOK vs
    # ~7bn real — a units/currency artifact — drove net-cash to 861% of mcap, EV
    # to -896bn, a false net_cash_returner). We anchor the check on YAHOO's own
    # totalCash (generally correct): a genuine cash+investments basis is a few x
    # narrow cash, and even a home-ccy/USD mismatch is <~15x — >25x is unambiguous
    # corruption. (We deliberately do NOT gate on master `assets`: that field can
    # itself be stale/wrong-period, and using it as the reference produced FALSE
    # positives — e.g. it wrongly flagged genuinely-below-book names. Yahoo is the
    # reference, not a possibly-stale master level.) On a mixed-currency line
    # (Yahoo serves mcap/EV in home ccy but cash/debt in USD) no clean replacement
    # exists, so NULL cash and its cash-derived fields: the row becomes
    # cash-UNKNOWN (net-cash archetypes correctly stop firing; the EV consistency
    # pass refills EV from Yahoo's internally-consistent mcap/EV pair).
    ...
    if int(_cash_impossible.sum()):
        ...
        recon["cash NULLED (impossible: > 25x Yahoo totalCash)"] = int(_cash_impossible.sum())
