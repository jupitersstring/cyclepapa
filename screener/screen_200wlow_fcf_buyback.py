"""
200-week-low + high normalized FCF yield + buyback machine screener.
Backend: EODHD All-In-One (you already pay for this).
...
Notes:
- "Normalized FCF yield" = mean(FCF / market_cap) over the trailing 5 fiscal years,
  recomputed using the CURRENT market cap. This avoids the trap where a transient
  earnings hit kills TTM FCF and the name screens out exactly when you want it.
...
- Capital structure reset and transience scores are NOT computed here. They are
  the qualitative overlay you apply by hand to the ~30-50 survivors.
"""
...
def weekly_prices(ticker: str, years: int = 5) -> Optional[pd.Series]:
    ...
    params = {"api_token": KEY, "period": "w", "fmt": "json",
...
def pct_above_200w_low(prices: pd.Series) -> Optional[float]:
    # Use the last ~260 weekly bars (5 years ≈ 260 weeks; "200-week" is close enough
    # and what most platforms display, but be explicit)
...
def fundamentals(ticker: str) -> Optional[Fundamentals]:
    ...
    for _, row in sorted(cf_y.items(), reverse=True)[:5]:
        ...
        if ocf is not None and capex is not None:
            fcf_years.append(float(ocf) + float(capex))  # capex is negative in EODHD
    if len(fcf_years) < 3:
        return None
    ...
    total_debt = float(latest_bs.get("shortLongTermDebtTotal") or
                       (float(latest_bs.get("shortTermDebt") or 0) +
                        float(latest_bs.get("longTermDebt") or 0)))
    cash = float(latest_bs.get("cashAndShortTermInvestments") or
                 latest_bs.get("cash") or 0)
...
def screen(args):
    ...
    df["composite"] = (
        0.30 * (df["norm_fcf_yield_5y"] / df["norm_fcf_yield_5y"].max()) +
        0.25 * (df["net_buyback_yield_ttm"].clip(lower=0) / df["net_buyback_yield_ttm"].clip(lower=0).max()) +
