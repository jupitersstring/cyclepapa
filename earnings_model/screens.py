...
def consensus_lagging(df: pd.DataFrame, top: int | None = 40,
                      min_analysts: int = 5, **elig) -> pd.DataFrame:
    ...
    adjusted to the trailing run-rate. Gated tightly to avoid artifacts:

    * ``consensus_gap_pct`` between -0.95 and -0.05 (forward 5-95% below trailing
    ...
    * exclude REITs (trailing EPS is NAV revaluation noise).
    ...
    if "consensus_gap_pct" not in e.columns:
        raise ValueError("consensus_gap_pct missing — refresh valuation for forwardEps/trailingEps")
