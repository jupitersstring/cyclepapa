...
def screen_one(ticker, df):
    ...
    mfi_series = df['mfi'].tail(104).dropna()
    if len(mfi_series) < 50: return None
    ...
    return {
        ...
        'weeks_since_low': bars_since_low,
...
for i in range(0, len(syms), chunk):
    ...
    try:
        data = yf.download(' '.join(batch), period='5y', interval='1wk',
...
out.to_csv('/tmp/tech_compression_weekly.csv', index=False)
